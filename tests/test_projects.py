...
def get_test_data():
    ...
    return {
        ...
        "token": "test-token",
        "url": "https://api.bitbucket.org/2.0/workspaces/test-workspace/projects/",
    }
...
def test_create_project_success(requests_mock):
    ...
    # Call the function
    create_project(data["workspace"], data["project_key"], data["name"], data["description"], data["token"])
...
def test_create_project_unauthorized(requests_mock):
    ...
    """
    data = get_test_data()
    data["token"] = "invalid-token"  # Simulate an invalid token
